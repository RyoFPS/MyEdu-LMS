...
class DashboardController extends Controller
{
    ...
    {
        $cacheKey = 'dashboard:admin:' . today()->format('Y-m-d');
    ...
    {
        $cacheKey = 'dashboard:teacher:' . $user->id . ':' . today()->format('Y-m-d');
        ...
        return Cache::remember($cacheKey, 60, function () use ($user) {
            ...
            $classes = $user->teachingClasses()
                ->withCount([
                    'students',
                    'attendances as today_present' => fn ($q) => $q->whereDate('date', today())->where('status', 'present'),
                    'attendances as today_absent'  => fn ($q) => $q->whereDate('date', today())->where('status', 'absent'),
                    'attendances as today_late'    => fn ($q) => $q->whereDate('date', today())->where('status', 'late'),
                    'attendances as today_excused' => fn ($q) => $q->whereDate('date', today())->where('status', 'excused'),
                ])
                ->get()
                ->map(fn ($c) => [
                    'id'             => $c->id,
                    ...
                    'today_present'  => $c->today_present,
                    'today_absent'   => $c->today_absent,
                    'today_late'     => $c->today_late,
                    'today_excused'  => $c->today_excused,
                ]);
    ...
    {
        $cacheKey = 'dashboard:student:' . $user->id . ':' . today()->format('Y-m-d');
